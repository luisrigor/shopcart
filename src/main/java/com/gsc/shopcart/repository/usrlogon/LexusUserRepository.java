...
public interface LexusUserRepository extends JpaRepository<LexusUser, Integer> {
    ...
    @Query(value = "SELECT ID_UTILIZADOR, NOME_UTILIZADOR FROM LEXUS_USERS WHERE OID_DEALER_PARENT=:oidDealerParent ORDER BY COLLATION_KEY_BIT(NOME_UTILIZADOR, 'UCA500R1_S1') ",
            nativeQuery = true)
